			 ...
			 "\n" . '<div class="profile">'.
				 "\n" . '<div class="profile-header">'.
					 ...
					 "\n" . '<ul class="profile-header-tab nav nav-tabs">'.
						 ...
						 "\n" . '<li class="nav-item"><a href="#" class="nav-link active" >Kho Xe</a></li>'.
...
echo  "\n" . '<div class="card-deck justify-content-md-center">';
	$query = mysqli_query($connect,"SELECT * FROM `vehicles` WHERE `sqlID` = '".$user['id']."'");
	$i = 1;
	while ( $data = mysqli_fetch_array($query) ) {
	
	$sql_veh = mysqli_query($connect,"SELECT * FROM `mt_cms_info_veh` WHERE `ID_Veh` = '".$data['pvModelId']."'");
	$infocar = mysqli_fetch_assoc($sql_veh);
	if($data['pvColor1'] > 0 && $data['pvColor2']  > 0){
		$color = '(<i class="cms-icon icon-car-1 color-'.$data['pvColor1'].'"></i>)(<i class="cms-icon icon-car-2 color-'.$data['pvColor2'].'"></i>)';
	}else{
		$color = 'None';
	}
	...
	if($data['pvLock'] == 0){
		$pvLock = 'None';
	}elseif($data['pvLock'] == 1){
		$pvLock = 'Khóa thủ công';
	}elseif($data['pvLock'] == 2){
		$pvLock = 'Khóa Điện';
	}else{
		$pvLock = 'Không rõ';
	}
				...
				"\n" . '<!-- begin card -->'.
					"\n" . '<div class="card">'.
						"\n" . '<div class="card-header text-center">'.
								...
								"\n" . '</div>'.
							"\n" . '<img class="card-img" src="'.$system['url'].'/Auth/Picture/Cars-'.$data['pvModelId'].'" alt="'.$infocar['Name_Veh'].'" />'.
								"\n" . '<div class="card-body">'.
							"\n" . '<h4 class="card-title text-center">Thông Tin Về Xe</h4><hr>'.
							"\n".'<div class="row card-text-cars">'.
				"\n".'<div class="col-4 col-sm-4 col-md-4">'.
							"\n".'<dl class="border-right">'.
 "\n".'<dt class="text-left">ID Cars</dt>'	.
 "\n".'<dd>'.$infocar['ID_Veh'].'</dd><hr>'.
  "\n".'<dt class="text-left">Xăng Cars</dt>'.
 "\n".'<dd>'.$data['pvFuel'].'</dd><hr>'.
"\n".'</dl>'.
"\n".'<span class="border-right-0"></span>'.
				"\n" . '</div>'.
				"\n".'<div class="col-4 col-sm-4 col-md-4">'.
							"\n".'<dl>'.
  "\n".'<dt class="text-center">Name Cars</dt>'.
  "\n".'<dd class="text-center">'.substr($infocar['Name_Veh'],'0','7').'...</dd><hr>'.
    "\n".'<dt class="text-center">Khóa Xe</dt>'.
  "\n".'<dd class="text-center">'.$pvLock.'</dd><hr>'.
"\n".'</dl>'.
				"\n" . '</div>'.
				"\n".'<div class="col-4 col-sm-4 col-md-4">'.
							"\n".'<dl class="border-left">'.
  "\n".'<dt class="text-center">Category</dt>'.
  "\n".'<dd class="text-center">'.$infocar['Category'].'</dd><hr>'.
    "\n".'<dt class="text-center">Color</dt>'.
  "\n".'<dd class="text-center">'.$color.'</dd><hr>'.
"\n".'</dl>'.
				"\n" . '</div>'.
							"\n" . '</div>'.
							"\n" . '</div>'.
