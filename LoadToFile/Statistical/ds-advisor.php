					...
					"\n" . '<table id="data-table-default" class="table table-striped table-bordered">'.
						"\n" . '<thead>'.
							"\n" . '<tr>'.
								...
								"\n" . '<th class="text-nowrap">Advisor Level</th>'.
								"\n" . '<th class="text-nowrap">Thời gian chơi</th>'.
								"\n" . '<th class="text-nowrap">Trạng thái</th>'.
							"\n" . '</tr>'.
						"\n" . '</thead>'.
						"\n" . '<tbody>';													
						$query = mysqli_query($connect,"SELECT * FROM accounts WHERE Helper >= 1 order by Helper DESC");
						$stt = 0;
						...
						if($data['Helper'] == '1'){
							$admin = 'Helper';
						}elseif($data['Helper'] == '2'){
							$admin = 'Advisor';
						}elseif($data['Helper'] == '3'){
							$admin = 'SA Advisor';	
						}elseif($data['Helper'] == '4'){
							$admin = 'Chief Advisor';	
						}else{
							$admin = 'Hack  Advisor';	
						}
